#  IBM watsonx.data Americas Enablement Bootcamp Env. Setup

**Technical Hands-on Lab: Provision Environment**

- [IBM watsonx.data Americas Enablement Bootcamp Env. Setup](#ibm-watsonxdata-americas-enablement-bootcamp-env-setup)
...
* Within `Prerequisites/auto-setup` folder create a subfolder `credentials` with 2 empty json files: `cos.json` and `postgres.json`. At later steps you will store there credentails for Cloud Object Storage `cos.json` and for Postgres database `postgres.json`.

![](attachments/2025-06-26-23-32-57-pasted-vscode.png)

* Create env file in `Prerequisites` folder: copy `./Prerequisites/auto-setup/env_setup.example` as `./Prerequisites/auto-setup/.env_setup` where you will during the next steps fill values for:
  * `CLOUD_API_KEY`
  * `WXD_REGION`
  * `WXD_INSTANCE_CRN`  
...
* Copy the key to `./Prerequisites/auto-setup/.env_setup` as a value for `CLOUD_API_KEY`
...
* Copy and paste the credentials  in `Prerequisites/credentials/auto-setup/cos.json`
...
Go to IBM Cloud `Resource List`, click on watsonx.data instance in the databases, scroll down and copy values in `./Prerequisites/auto-setup/.env_setup`:
...
From [Boxfolder](https://ibm.ent.box.com/folder/322609163013) `postgres.json` file copy all content into `Prerequisites/auto-setup/credentials/postgres.json`

Now you should have `./Prerequisites/auto-setup/.env_setup` ready to run `Auto-setup.ipynb` script.
...
* Make sure you have opened `watsonx.data-bootcamp-2025` in VSCode terminal
...
```sh
pip install -r ./Prerequisites/auto-setup/requirements_autosetup.txt
```

## 4.2 Run Jupyter Notebook with the script
*  Open `./Prerequisites/auto-setup/Auto-setup.ipynb`
*  Select kernel as `venv_prep`
*  Run All
*  As an output you should also get an output with some configuration prefilled for  your `.env_all` file -> `Prerequisites/auto-setup/.env_output`
...
1. Add Policy to grant access to iceberg table

Switch to Policies tab, click Add Policy

Policy name: iceberg

Change policy status after creation to Active
...
## 7 Data upload
Folders `Prerequisites/auto-setup/data` and `Prerequisites/auto-setup/pdfs` contain csv/json files for the second lab (watsonx.ai path) and pdfs for Lab4 - Rag Agent. If you want to add those files to corresponding COS buckets, you need to run the last part of Jupyter Notebook: **Add initial data to COS from local directory**. 
:warning: Make sure files are available at those local directories.